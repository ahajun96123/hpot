...
import com.jkl.hpot.util.ChatUtil;
import com.jkl.hpot.vo.*;
...
public class WebSocket extends TextWebSocketHandler {
	...
	public void afterConnectionEstablished(WebSocketSession session) throws Exception {
		...
		chatService.updateRoomMember(new ChatMemberVO(0, userId, "", priroom));
...
//통신 연결끊었을때실행
	...
	public String JsonData(String id, Object msg) {
		JsonObject jsonObject = Json
				.createObjectBuilder().add("message", "<i class='user icon'></i>"
						+ "<a href='#none' onclick=\"insertWisper('" + id + "')\"><b>[" + id + "]</b></a> : " + msg)
				.build();
	...
	public String JsonWisper(String fromId, Object msg) {
		JsonObject jsonObject = Json.createObjectBuilder().add("message", "<a href='#none' onclick=\"insertWisper('"
				+ fromId + "')\">" + "<i class='user icon'></i>" + " <b style='color:green'>[" + // 아이디를 클릭하게되면 귓속말아이디
																									// 세팅
